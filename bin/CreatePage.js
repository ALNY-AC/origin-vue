    ...
    if (program.name) {
        ...
        fs.unlink(newPathFold, function () {
            fs.mkdir(newPathFold, function () {
                fs.readFile(path.join(__dirname, './temp/page/page.vue'), 'utf8', function (err, vueFiles) {
                    fs.readFile(path.join(__dirname, './temp/page/page.js'), 'utf8', function (err, jsFiles) {
                        fs.readFile(path.join(__dirname, './temp/page/page.scss'), 'utf8', function (err, scssFiles) {
                            ...
                            fs.writeFile(newPathFold + '/' + name + '.vue', _vueFiles, 'utf8', function (err) {
                                fs.writeFile(newPathFold + '/' + name + '.js', _jsFiles, 'utf8', function (err) {
                                    fs.writeFile(newPathFold + '/' + name + '.scss', _scssFiles, 'utf8', function (err) {
                                        console.log(styles.greenBG.join('%s'), '创建页面级成功：' + process.cwd() + "\\" + program.name);
                                    });
                                });
                            });
